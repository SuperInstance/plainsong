...
**Applies to Plainsong:** Your plain-text DSL can compile to this hierarchical tokenization, not just raw MIDI. This lets Plainsong serve as a human-readable front-end for LLM-based composition, where the compiler emits both MIDI and the token stream for model fine-tuning.
...
**Applies to Plainsong:** Your stigmergy module can be embedded as a Plainsong directive (e.g., `@pheromone:lead-guitar-weight=0.7`). The compiler then translates these into bid utilities for the agent swarm, making the notation a coordination language, not just a score.
...
**Applies to Plainsong:** Plainsong can become the *authoring syntax* for these meta-channels. Example: `@intent:question` compiles to a rising minor-second motif in the SWMIDI-8 payload. This turns your notation into a human-writable protocol for agent-to-agent semantics.
