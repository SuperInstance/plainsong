...
- owner `SuperInstance`, repository `tapscript-studio`
...
| `spec_files/*.toml` | the specs `tapscript spec` runs |

Anything the code reads at runtime must be **inside** `tapscript/`, because a
...
`tapscript spec` reported "no specs found" to everybody who had not cloned the
...
/tmp/relcheck/bin/tapscript --version
/tmp/relcheck/bin/tapscript new "Smoke Test" -o smoke.tap
/tmp/relcheck/bin/tapscript compile smoke.tap -o smoke.mid
/tmp/relcheck/bin/tapscript spec
```
...
"No module named tapscript" about a package that installed perfectly well.
