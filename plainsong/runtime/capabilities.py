...
CLI prints it (``plainsong doctor``), the renderer picks backends from it, and
...
def probe_soundfonts() -> Capability:
    found: list[str] = []
    env_hint = os.environ.get("PLAINSONG_SOUNDFONT")
    ...
    return Capability(
        ...
        remedy="install a General MIDI soundfont, or set PLAINSONG_SOUNDFONT to one",
        data={"found": found[:8]},
    )
    ...
    for name, argv in PLAYBACK_COMMANDS:
        ...
        if path:
            return Capability(
                ...
                present=True,
                detail=path,
                unlocks="`plainsong play` straight from the terminal",
                data={"command": argv},
            )
...
def probe_network() -> Capability:
    ...
    offline = os.environ.get("PLAINSONG_OFFLINE", "").lower() in {"1", "true", "yes"}
    return Capability(
        name="network",
        present=not offline,
        kind="informational",
        ...
        remedy="unset PLAINSONG_OFFLINE to allow outbound calls",
...
def probe_host_agent() -> Capability:
        ...
        for env_name in env_names:
            if os.environ.get(env_name):
                return Capability(
                    name="host_agent",
                    present=True,
                    detail=os.environ.get("PLAINSONG_HOST_AGENT") or host,
    ...
    return Capability(
        ...
        remedy="set PLAINSONG_HOST_AGENT when driving this from another agent",
    )
