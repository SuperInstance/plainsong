...
1. An explicit environment variable (``PLAINSONG_CONFIG_DIR`` and friends).
2. A project-local ``.plainsong/`` directory, if the current working directory
...
PROJECT_MARKERS = (".plainsong", "plainsong.toml", "pyproject.toml", ".git")
...
class Paths:
    ...
    def __init__(self, project_root: Path | None = None) -> None:
        ...
        self.config_dir = _env_path("PLAINSONG_CONFIG_DIR") or config
        self.data_dir = _env_path("PLAINSONG_DATA_DIR") or data
        self.state_dir = _env_path("PLAINSONG_STATE_DIR") or state
        self.cache_dir = _env_path("PLAINSONG_CACHE_DIR") or cache

    # -- files ---------------------------------------------------------------

    @property
    def config_file(self) -> Path:
        override = _env_path("PLAINSONG_CONFIG")
        if override:
            return override
    ...
    def project_config_file(self) -> Path | None:
        """``.plainsong/config.toml`` beside the project, if there is a project."""
        if not self.project_root:
            return None
        return self.project_root / ".plainsong" / "config.toml"
    ...
    def workspace(self) -> Path:
        ...
        """
        override = _env_path("PLAINSONG_WORKSPACE")
        if override:
            return override
        if self.project_root:
            return self.project_root / ".plainsong" / "workspace"
    ...
    def bridge_dir(self) -> Path:
        ...
        override = _env_path("PLAINSONG_BRIDGE_DIR")
        if override:
            return override
