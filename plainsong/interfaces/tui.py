...
def run_tui(config: Config | None = None, path: str = "") -> int:
    ...
    try:
        ...
        print("Or use `plainsong serve` for the web interface, which needs nothing.")
        return 1
...
def _draw(screen, state: TuiState, show_help: bool) -> None:
    ...
    title = f" plainsong  --  {len(state.visible())} pieces "
...
def _transpose(screen, state: TuiState) -> None:
    ...
    try:
        ...
        return
    target = state.config.paths.output_dir / f"{entry.name}-{key}.song"
