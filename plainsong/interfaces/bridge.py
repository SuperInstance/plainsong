...
When plainsong is configured with the ``host`` provider in file mode, it drops
...
surrounding agent -- or a person -- answers it.
    ...
    plainsong bridge answer <id> --text "..."
...
def run_bridge(args, config: Config, out) -> int:
    ...
    if action == "answer":
        if not args.request_id:
            out.fail("usage: plainsong bridge answer <request-id> --text '...'")
            return 2
        text = args.text
        if not text:
            import sys
        ...
        try:
            while True:
                for item in pending_bridge_requests(bridge_dir):
                    ...
                    print(f"\n# answer with: plainsong bridge answer {request_id} --text '...'\n")
                time.sleep(0.5)
