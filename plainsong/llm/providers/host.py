...
REQUEST_SENTINEL = "<<<PLAINSONG_BRIDGE_REQUEST>>>"
RESPONSE_SENTINEL = "<<<PLAINSONG_BRIDGE_RESPONSE>>>"
...
class HostProvider(Provider):
    ...
    def __init__(self, info, api_key: str = "", model: str = "", **options) -> None:
        ...
        self.mode = str(options.get("host_mode") or os.environ.get("PLAINSONG_HOST_MODE") or "command")
        self.command = str(options.get("host_command") or os.environ.get("PLAINSONG_HOST_COMMAND") or "")
    ...
    def _run_command(self, prompt: str) -> str:
        ...
        if not argv:
            raise ProviderError(
                ...
                provider=self.id,
                hint='set one with `plainsong config set llm.host_command "claude -p"`',
            )
        try:
            ...
            raise ProviderError(
                ...
                provider=self.id,
                hint="check `plainsong config get llm.host_command`",
            ) from exc
    ...
    def check(self) -> tuple[bool, str]:
        if self.mode == "command" and not self.command:
            return False, 'no host command set (`plainsong config set llm.host_command "claude -p"`)'
        if self.mode == "file":
