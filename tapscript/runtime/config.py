...
# Environment overrides: TAPSCRIPT_<SECTION>_<KEY>, plus a few friendly aliases.
ENV_ALIASES = {
    "TAPSCRIPT_PROVIDER": ("llm", "provider"),
    "TAPSCRIPT_MODEL": ("llm", "model"),
    "TAPSCRIPT_PORT": ("web", "port"),
    "TAPSCRIPT_HOST": ("web", "host"),
}
...
class Config:
    ...
    def save(self, path: Path | None = None) -> Path:
        ...
        header = (
            "# tapscript configuration\n"
            "# Values omitted here fall back to the built-in defaults.\n"
            "# See `tapscript config --explain` for the full resolved set.\n\n"
        )
...
def load_config(overrides: dict[str, Any] | None = None, paths: Paths | None = None) -> Config:
    ...
    for section, values in DEFAULTS.items():
        for key, reference in values.items():
            env_name = f"TAPSCRIPT_{section.upper()}_{key.upper()}"
            if env_name in os.environ:
                env_overlay.setdefault(section, {})[key] = _coerce(os.environ[env_name], reference)
    if env_overlay:
