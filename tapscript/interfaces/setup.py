...
def run_setup(config: Config, out, provider_id: str = "", non_interactive: bool = False) -> int:
    ...
    out.head("tapscript setup")
    out.say()
    out.say("The compiler works with no setup at all:")
    out.dim("    tapscript new song.tap && tapscript compile song.tap --play")
    out.say()
    ...
    if info is None:
        ...
        out.dim("see `tapscript providers` for the list")
        return 1
    ...
    if info.id == "host":
        ...
        if _interactive() and not non_interactive:
            out.say()
            out.say("How should tapscript reach your agent?")
    ...
    if ok:
        ...
        out.say()
        out.say("try it:")
        out.dim('    tapscript agent "write a slow waltz in D minor"')
        return 0

    out.warn(detail)
    out.dim("the settings were saved; fix the problem above and run `tapscript providers --check`")
    return 1
...
def _ask_provider(catalog, out) -> str:
    ...
    out.say(f"  {len(options) + 1}. something else (see `tapscript providers`)")
