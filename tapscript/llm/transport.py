...
USER_AGENT = "tapscript/1.0 (+https://github.com/SuperInstance/tapscript-studio)"
...
def _classify(status: int, body: str, provider: str) -> ProviderError:
    ...
    if status in (401, 403):
        hint = f"check the API key for {provider} (`tapscript providers --check`)"
    elif status == 404:
        hint = "the model name may not exist for this provider (`tapscript providers --models`)"
    elif status == 429:
