...
3. ``credentials.toml`` in the config directory, written by ``tapscript setup``.
...
def store_key(provider_id: str, api_key: str, paths: Paths | None = None) -> Path:
    ...
    header = (
        "# tapscript credentials\n"
        "# Written by `tapscript setup`. Keep this file out of version control.\n"
