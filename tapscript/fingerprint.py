...
established stopped being checked the moment the script was closed. `tapscript
...
Anybody with a folder of `.tap` files they care about has the same problem we
do, and no access to our test suite:

    tapscript fingerprint mysongs > before.txt
    pip install --upgrade tapscript
    tapscript fingerprint mysongs > after.txt
...
def fingerprint_paths(paths: list[str]) -> list[FileFingerprint]:
    """Fingerprint every `.tap` file under each path, in a stable order."""
    found: list[Path] = []
    for entry in paths:
        candidate = Path(entry)
        if candidate.is_dir():
            found.extend(sorted(candidate.rglob("*.tap")))
