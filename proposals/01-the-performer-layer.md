...
music is. Plainsong compiles a chart to MIDI correctly — every note where the
...
anyone with a MIDI file and no interest in Plainsong at all — that is most
...
**Only stage 1 is Plainsong.** Stages 2 and 3 are what make stage 1 worth
...
`plainsong/perform/solve.py` already treats a written beat as the moment a
...
Plainsong goes in over the wire, an agentic pass-through performs it, and MIDI
...
The point is that Plainsong is a format an *agent* can write fluently — the
several thousand `.song` files in this repository were generated, which is the
...
chairs. `plainsong-mcp` already has the session layer for many agents holding
...
Nothing here starts until `plainsong` and `plainsong-mcp` are merged,
